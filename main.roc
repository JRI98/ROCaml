	...
	match l {
		...
		[e1, e2, .. as rest] => {
			...
			if e1 == e2 {
				...
				List.prepend(rest_compression, e1)
			}
		}
	}
}
		...
		match l {
			...
			[e, .. as rest] => {
				...
				match rest_pack {
					...
					[[rpe, ..] as p, .. as rprest] => {
						if e == rpe {
							List.prepend(rprest, List.append(p, e))
						} else {
							List.prepend(rest_pack, [e])
						}
					}
		...
		[e, .. as rest] => {
			...
			match encode_rest {
				...
				[er, .. as errest] => {
					...
					if e == element {
						List.prepend(errest, Many(number + 1, e))
					} else {
						List.prepend(encode_rest, One(e))
					}
				}
			}
		}
